...
nquery(document).ready(function()
{
	...
	{
		if(nquery('#nickx-gallery .slick-list.draggable .slick-track > li').length <=4)
		{
			nquery('#nickx-gallery .slick-list.draggable .slick-track').addClass('techno_stop_slide');
		}
		if(nquery('#product_video_iframe').length > 0 && nquery('#product_video_iframe').attr('video-type')=='youtube'){
		    ...
		    nquery('#product_video_iframe').attr('src','//www.youtube.com/embed/'+iframe_src+'?rel='+rel+'&showinfo=0&enablejsapi=1');
		    nquery('#product_video_iframe_light').attr('href','//www.youtube.com/embed/'+iframe_src+'?enablejsapi=1&wmode=opaque&rel='+rel);
		    	...
		    	xhr.onload = function(e)
		    	{
		      		if(xhr.readyState === 4)
		      		{
		        		if(xhr.status === 200)
		        		{
				          	...
				          	nquery('.product_video_img').attr('src',thumbnail_small);
		        		} 
		        		else
		        		{
	...
	if(nquery('.nickx-slider-for').length > 0)
	{
		...
		nquery(".nickx-slider-for").on("beforeChange", function (){
		...
		if(wc_prd_vid_slider_setting.nickx_show_zoom == 'yes'){
			nquery('.nickx-slider-for .slick-slide').zoom();
		}
		nquery('.nickx-slider-for .slick-track').addClass('woocommerce-product-gallery__image single-product-main-image');
		nquery('.nickx-slider-nav .slick-track').addClass('flex-control-nav');
		nquery('.nickx-slider-nav .slick-track li img').removeAttr('srcset');		
		nquery('.variations select').change(function(){
			nquery('.nickx-slider-nav').slick('slickGoTo', 0);
			window.setTimeout(function() 
			{
				if(wc_prd_vid_slider_setting.nickx_show_zoom == 'yes')
				{
					nquery('.nickx-slider-for .slick-track .slick-current').zoom();
				}
			},20);
		});				
		if(nquery('.slick-cloned .nickx-popup').remove()){
